...
export default function CounsellorAuthHeader({ headerLeftOffset }) {
  ...
  const handleMarkAllNotificationsRead = () => {
    ...
    setNotificationUnreadCount(0);
    clearNotificationUnreadCount();
    markAllNotificationsAsRead(userInfo).finally(() => refreshNotificationInboxForUser(userInfo).catch(() => {}));
  };
  ...
  const handleNotificationItemClick = (item) => {
    ...
    if (item?.id) {
      ...
      markNotificationAsRead({ notificationId: item.id, user: userInfo }).finally(() =>
        refreshNotificationInboxForUser(userInfo).catch(() => {})
      );
    }
                ...
                >
                  ...
                  <List sx={{ p: 0, maxHeight: 420, overflowY: "auto" }}>
                    {notificationItems.length === 0 ? (
                      <ListItem>
                        <ListItemText primary="Chưa có thông báo" secondary="Thông báo mới sẽ hiện tại đây." />
                      </ListItem>
                    ) : (
                      notificationItems.map((item) => (
                        ...
                        >
                          ...
                          <Box sx={{ minWidth: 0, flex: 1 }}>
                            <Typography sx={{ fontSize: 16, fontWeight: item.read ? 700 : 800, color: "#0b1220", lineHeight: 1.3, mb: 0.35 }}>
                              {item.title}
                            </Typography>
                            <Typography sx={{ fontSize: 13.5, fontWeight: 400, color: "#1e293b", lineHeight: 1.45 }}>
                              {item.body}
                            </Typography>
                            <Typography sx={{ fontSize: 12, color: "#64748b", mt: 0.75 }}>{formatNotificationTime(item.createdAt)}</Typography>
                          </Box>
                          {!item.read && <Box sx={{ width: 10, height: 10, borderRadius: "50%", bgcolor: "#3b82f6", mt: 1 }} />}
                        </ListItemButton>
                      ))
                    )}
                  </List>
